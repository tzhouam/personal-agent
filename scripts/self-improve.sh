...
#
# Isolation: ALL work happens in a throwaway git worktree under DATA_DIR — never
# the live checkout — so it can't disturb the running daemon or wipe uncommitted
# edits. Safety: refuses any change touching .env/.git/credentials; gates every
# change on the full test suite (run against the worktree via PYTHONPATH);
# discards the branch if red; opens a PR (not auto-merged). Opus 4.8 is reached
# by clearing the ANTHROPIC_* proxy env so the CLI uses the logged-in Anthropic
# account, not the DeepSeek/MiMo endpoint.
set -uo pipefail
...
# 1. evidence first; skip the (costly) Opus call on a quiet day
"$PY" "$REPO/scripts/self_improve_evidence.py" 2 > "$BRIEF" 2>>"$LOG"
if [ ! -s "$BRIEF" ]; then
  echo "no friction/perf signals in the last 2 days — nothing to improve"; exit 0
fi
...
if ! ( cd "$WT" && PYTHONPATH="$WT/src" "$PY" -m pytest test/ -q >/tmp/si-pytest.log 2>&1 ); then
  ...
  announce "⚠️ 每日自我改进 $DAY：Opus 提了改动但测试未过，已丢弃。"; exit 1
fi
echo "tests green"
...
chat friction, and task failures. Tests pass. **Review before merging — not auto-merged.**
Evidence and Opus's own summary are in \`~/.personal-agent/self-improve.log\`."
