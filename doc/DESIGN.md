...
actually act on them. Feeds fetch with four bounded workers and short per-phase
HTTP timeouts, but results are reassembled in configured source order. Three
consecutive failures put only that source+policy fingerprint into a deployment-shared
72-hour cooldown with one half-open probe; changing its URL/config bypasses the
old state. Relevance scoring uses 20-item structured-output batches with at
most two calls in flight, so one malformed/truncated batch falls back locally
instead of discarding the whole pool. GitHub triage uses the same pattern in
15-item batches (up to its existing 60-notification cap).
...
actions; no explicit ratings. Digest/research artifacts also record requested,
model-completed, and deterministic-fallback counts plus a numeric `degraded`
flag, so a successful phase cannot hide a total model fallback. Full catalog
and the research behind each metric:
